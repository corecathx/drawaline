...
class TextInput extends FlxSpriteGroup {
    ...
    public static function show(x:Float, y:Float, w:Int, h:Int, text:String,
                                onConfirm:String->Void, ?onCancel:Void->Void) {
        instance._open(x, y, w, h, text, onConfirm, onCancel);
    }

    function _open(x:Float, y:Float, w:Int, h:Int, text:String,
                   onConfirm:String->Void, ?onCancel:Void->Void) {
    ...
    function close(confirm:Bool) {
        ...
        if (confirm) {
            if (_onConfirm != null) _onConfirm(_input.text);
        } else {
