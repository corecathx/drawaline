...
class LayerList extends FlxSpriteGroup {
    ...
    public function rebuild() {
        ...
        for (i in 0...canvas.layers.length) {
            var item = new LayerItem(
                2, i * (ITEM_HEIGHT + ITEM_GAP) + ITEM_GAP,
                listWidth - 4, ITEM_HEIGHT,
                canvas.layers[i], canvas, this
            );
    ...
    function _applyClip() {
        ...
        for (member in itemContainer.members) {
            ...
            sprite.clipRect = (clipTop > 0 || clipBottom > 0)
                ? new FlxRect(0, clipTop, sprite.width, ITEM_HEIGHT - clipTop - clipBottom)
                : null;
        }
    }
}
...
class LayerItem extends FlxSpriteGroup {
    ...
    public function new(x:Float, y:Float, width:Int, height:Int, layer:Layer, canvas:Canvas, parent:LayerList) {
        ...
        add(upBtn); add(downBtn); add(visibilityBtn); add(deleteBtn);

        upBtn.onClick   = () -> { if (canvas.reorderLayer(layer.layerID, -1)) parent.pendingRebuild = true; };
        downBtn.onClick = () -> { if (canvas.reorderLayer(layer.layerID,  1)) parent.pendingRebuild = true; };
        visibilityBtn.onClick = () -> {
            canvas.setLayerVisibility(layer.layerID, !layer.visible);
            visibilityBtn.setIcon(layer.visible ? "visibility" : "visibility_off");
        };
        deleteBtn.onClick = () -> { if (canvas.removeLayer(layer.layerID)) parent.pendingRebuild = true; };
    }
    ...
    override function update(elapsed:Float) {
        ...
        if (thumbTimer >= THUMB_REFRESH_INTERVAL) {
            thumbTimer = 0;
            if (layer.dirty) _refreshThumbnail();
        }
        ...
        if (FlxG.mouse.overlaps(bg, cam) && !overBtn) {
            if (!focused) bg.color = Colors.buttonHover;
            if (FlxG.mouse.justPressed) {
                canvas.setFocus(layer.layerID);

                var now:Float = haxe.Timer.stamp();
                if (now - _lastClickTime < DOUBLE_CLICK_TIME) {
                    TextInput.show(x + (label.x - x), y + (label.y - y), Std.int(label.fieldWidth), Std.int(label.height),
                        layer.layerName,
                        (v) -> { layer.layerName = label.text = v.length > 0 ? v : layer.layerName; },
                        null
                    );
                }
                _lastClickTime = now;
